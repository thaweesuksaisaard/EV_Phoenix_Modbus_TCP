...
EthernetServer serverEthernet(502);
EthernetClient client;

//const char* ssid = "BluestoneEV";
char ssid[18] = {'B', 'S', '_', 'E', 'V'};
const char* password = "bluestone2022";

const char* www_username = "bluestone";
const char* www_password = "bluestone";

uint16_t maxLenght = 12;
// Declare object modbus meter and ev
MODBUS modbusMeter;
MODBUS modbusEV;
uint8_t _buffers1[100];
uint8_t _buffers2[100];
uint8_t _buffers3[100];
uint8_t _bufferFN06[20];
uint8_t _bufferFN05[20];
uint8_t _bufferRestart[20];

//******************************************************************************************
uint8_t _buffers_meter_parameter[100];
uint8_t _buffers_meter_energy[50];
uint8_t _buffers_ev_s1s2[50];
uint8_t _buffers_ev_status[100];
uint8_t _buffers_ev_mode[50];
uint8_t _buffers_ev_current[50];
uint8_t _buffers_ev_coil[50];
...
byte modbusTcpResponse_readModbus[100];
...
uint8_t id_TCP[4];
bool success;
bool flag_RFID = false;
bool flag_buffer;

// Declare function
void readModbus(byte dataModbusTcp[]);
void illegalDataAddress(byte dataModbusTcp[]);
void illegalDataFunction(byte dataModbusTcp[]);
void setFN06(byte dataModbusTcp[]);
void setFN16(byte dataModbusTcp[]);
void setRFID(byte dataModbusTcp[]);
void setIP(byte dataModbusTcp[]);
void restartEV(byte dataModbusTcp[]);
void restartESP(byte dataModbusTcp[]);
// Declare function LED
...
void onJavaScript(void) {
  ...
  serverWiFI.setContentLength(jquery_min_js_v3_2_1_gz_len);
  serverWiFI.sendHeader(F("Content-Encoding"), F("gzip"));
  serverWiFI.send_P(200, "text/javascript", jquery_min_js_v3_2_1_gz, jquery_min_js_v3_2_1_gz_len);
}
...
void readEthernet(void *parameter) {
  Serial.println("Start task 1");
  while (1)
  {
    //Serial.println("Task1");
    //vTaskDelay(1000 / portTICK_PERIOD_MS);
    client = serverEthernet.available();
    if (client)
    {
      ...
      while (client.connected())
      {
        ...
        //Serial.print("Usage stack ");  Serial.println(uxHighWaterMark);
        if (client.available())
        {
          byte c = client.read();
          ...
          if (i == 8)
          {
            if (modbusTcpRequest[7] != 0x10) {
              maxLenght = 12;
            }
          }

          if (i == 12 && modbusTcpRequest[7] == 0x10)
          {
            maxLenght = 13 + (modbusTcpRequest[11] * 2);
          }
          ...
          if (i == maxLenght)
          {
            ...
            Serial.println();
            //Serial.print("Task Priority "); Serial.println(uxTaskPriorityGet(NULL));

            //Serial.print("max = ");
            //Serial.println(maxLenght);
            ...
            switch (modbusTcpRequest[7])   // check function such as 0x04, 0x01
            {
              case 0x04:
                ...
                {
                  illegalDataAddress(modbusTcpRequest);
                }
                break;
              case 0x05:
                if (checkAddress == 0x5208)
                {
                  restartEV(modbusTcpRequest);
                }
                else if (checkAddress == 0x5209)
                {
                  restartESP(modbusTcpRequest);
                }
                else
                {
                  illegalDataAddress(modbusTcpRequest);
              ...
              case 0x06:
                flag_task = true;
                setFN06(modbusTcpRequest);
                flag_task = false;
                break;
              case 0x10:
                flag_task = true;
                if (checkAddress == 0x7552)
                {
                  setRFID(modbusTcpRequest);
                }
                else if (checkAddress == 0x7554)
                {
                  setIP(modbusTcpRequest);
                }
                else {
                  setFN16(modbusTcpRequest);
                }
                flag_task = false;
                break;
              default:
                illegalDataFunction(modbusTcpRequest);
                break;
            }
...
void setup() {
  ...
  vTaskDelay(1000 / portTICK_PERIOD_MS);
  // Initialize meter
  modbusMeter.init(1, Serial2);
  // Initialize EV Controller
  modbusEV.init(2, Serial2);

  // Initialize (cs) in spi module ethernet
  Ethernet.init(5);
  Serial.begin(115200);
  vTaskDelay(1000 / portTICK_PERIOD_MS);
  Serial.println("Ethernet TCP Server");

  // EEPROM keep ip tpc address
  EEPROM.begin(32);
  for (uint8_t i = 0; i < 4; i++) {
    ip_TCP[i] = EEPROM.read(i);
    Serial.print(ip_TCP[i]);  Serial.print(".");
  }
  Serial.println();

  if ((ip_TCP[0] == 0 && ip_TCP[1] == 0 && ip_TCP[2] == 0 && ip_TCP[3] == 0)
      || (ip_TCP[0] == 0xFF && ip_TCP[1] == 0xFF && ip_TCP[2] == 0xFF && ip_TCP[3] == 0xFF))
  {
    ip_TCP[0] = 192;  ip_TCP[1] = 168;  ip_TCP[2] = 4;  ip_TCP[3] = 1;
  }
  IPAddress ip( ip_TCP[0], ip_TCP[1], ip_TCP[2], ip_TCP[3]);

  // Start the Ethernet connection and the server:
  WiFi.macAddress(mac);   // get mac wifi mode STA configure mac ethernet following
  ...
  ledcAttachPin(led_r, redChannel);
  ledcAttachPin(led_g, greenChannel);
  ledcAttachPin(led_b, blueChannel);

  turquoise_stateA();

  // Create interrupt switch
  attachInterrupt(sw_button, isr, FALLING);
  // Connect to WiFi network
  //WiFi.softAP(ssid,password,1,0,1);

  String mac_ssid = WiFi.macAddress();
  Serial.print("mac ssid"); Serial.println(mac_ssid);

  for (uint8_t a = 5, b = 5; a <= 16; a++) {
    ssid[a] = mac_ssid[a - b];
    if (a % 2 == 0)
      b--;
  }
  ...
  serverWiFI.on("/jquery-3.2.1.min.js", HTTP_GET, onJavaScript);
  /*return index page which is stored in serverIndex */
  serverWiFI.on("/logout", HTTP_GET, []() {
    serverWiFI.sendHeader("Connection", "close");
    serverWiFI.send(401, "text/plain", "Log out OK");
  });
  serverWiFI.on("/updateFirmware", HTTP_GET, []() {
     if (!serverWiFI.authenticate(www_username, www_password)) {
      return serverWiFI.requestAuthentication();
    }
    serverWiFI.sendHeader("Connection", "close");
    serverWiFI.send(200, "text/html", serverIndex);
  });
  /*handling uploading firmware file */
  serverWiFI.on("/update", HTTP_POST, []() {
    serverWiFI.sendHeader("Connection", "close");
    serverWiFI.send(200, "text/plain", (Update.hasError()) ? "FAIL" : "OK");
    ESP.restart();
  }, []() {
...
void loop() {
  ...
  if ((millis() - lastTime) >= period)
  {
    ...
    u = 0;
    //Serial.print("Loop Priority "); Serial.println(uxTaskPriorityGet(NULL));
    flag_buffer = true;
    flag_loop = true;
    // meter parameter
    vTaskDelay(10 / portTICK_PERIOD_MS);
    if (modbusMeter.readMultipleParameter(0x0000, 18, _buffers_meter_parameter, 100)) {
    } else {
      memset(_buffers_meter_parameter, 0, sizeof(_buffers_meter_parameter));
    }
    // meter energy
    vTaskDelay(10 / portTICK_PERIOD_MS);
    if (modbusMeter.readMultipleParameter(0x0156, 2, _buffers_meter_energy, 100)) {
    } else {
      memset(_buffers_meter_energy, 0, sizeof(_buffers_meter_energy));
    }
    // EV read Configuration switch s1,s2 address 2000-2001
    vTaskDelay(10 / portTICK_PERIOD_MS);
    if (modbusEV.readMultipleParameter(0x07D0, 2, _buffers_ev_s1s2, 100)) {
    } else {
      memset(_buffers_ev_s1s2, 0, sizeof(_buffers_ev_s1s2));
    }
    // read address 24000 - 240025
    vTaskDelay(10 / portTICK_PERIOD_MS);
    if (modbusEV.readMultipleParameter(0x5DC0, 26, _buffers_ev_status)) {
      vehicleStatus = _buffers_ev_status[3] << 8 | _buffers_ev_status[4];
      ...
      if (_buffers_ev_status[53] != 0x00 || _buffers_ev_status[54] != 0x00)
        red_stateError();
    } else {
      memset(_buffers_ev_status, 0, sizeof(_buffers_ev_status));
      red_stateError();
    }
    // read en address 4000
    vTaskDelay(10 / portTICK_PERIOD_MS);
    if (modbusEV.readMultipleParameter(0x0FA0, 1, _buffers_ev_mode, 100)) {
    } else {
      memset(_buffers_ev_mode, 0, sizeof(_buffers_ev_mode));
    }
    // read setting current permision address 22000
    vTaskDelay(10 / portTICK_PERIOD_MS);
    if (modbusEV.readMultipleParameter(0x55F0, 1, _buffers_ev_current, 100)) {
    } else {
      memset(_buffers_ev_current, 0, sizeof(_buffers_ev_current));
    }
    // read coil 20000
    vTaskDelay(10 / portTICK_PERIOD_MS);
    if (modbusEV.readSingleCoil(0x4E20, _buffers_ev_coil, 100)) {
    } else {
      memset(_buffers_ev_coil, 0, sizeof(_buffers_ev_coil));
    }

    flag_loop = false;

    for (k = 9; k <= 44; k++)     // meter parameter
    {
      data_TCP[k] = _buffers_meter_parameter[k - 6];
    }
    for (k = 45; k <= 48; k++)    // meter energy
    {
      data_TCP[k] = _buffers_meter_energy[k - 42];
    }
    // EV Configuratkon swktch S1, S2 address 2000-2001
    for (k = 49; k <= 52; k++)
    {
      data_TCP[k] = _buffers_ev_s1s2[k - 46];
    }

    // Ev address 24000, 24003, 24004, 24005, 24017, 24018, 24019 , 24020, 24025
    // Status vehkcle address 24000
    for (k = 53; k <= 54; k++)
      data_TCP[k] = _buffers_ev_status[k - 50];
    // Charging current via signal  CP in A
    for (k = 55; k <= 56; k++)
      data_TCP[k] = _buffers_ev_status[k - 46];
    // Status input
    for (k = 57; k <= 58; k++)
      data_TCP[k] = _buffers_ev_status[k - 46];
    // Status output
    for (k = 59; k <= 60; k++)
      data_TCP[k] = _buffers_ev_status[k - 46];
    // Time connct state B, C, D
    for (k = 61; k <= 68; k++)
      data_TCP[k] = _buffers_ev_status[k - 24];
    // Error
    data_TCP[69] = _buffers_ev_status[53];
    data_TCP[70] = _buffers_ev_status[54];

    // Mode en address 4000
    data_TCP[71] = _buffers_ev_mode[3];
    data_TCP[72] = _buffers_ev_mode[4];
    // Max current permisible address 2200
    data_TCP[73] = _buffers_ev_current[3];
    data_TCP[74] = _buffers_ev_current[4];
    // Coil address 20000
    data_TCP[75] = 0x00;
    data_TCP[76] = _buffers_ev_coil[3];
    Serial.println("Finish");
  }
  else if (flag_RFID == true && flag_task == false)
  {
    Serial.print("Read RFID = "); Serial.println(u);
    if (rfid.getIdHex(id, 50)) {
...
void readModbus(byte dataModbusTcp[]) {
  // Clear array
  //memset(modbusTcpResponse_readModbus, 0 , sizeof(modbusTcpResponse_readModbus));

  uint8_t i, j;
  uint8_t lengths = modbusTcpRequest[11] * 2;

  for (uint8_t n = 0; n <= 3; n++)
    modbusTcpResponse_readModbus[n] = dataModbusTcp[n];

  modbusTcpResponse_readModbus[4] = 0x00;   // Length field high byte
  modbusTcpResponse_readModbus[5] = lengths + 3;   // Length field low byte
  modbusTcpResponse_readModbus[6] = dataModbusTcp[6];   // Slave
  modbusTcpResponse_readModbus[7] = dataModbusTcp[7];   // Function
  modbusTcpResponse_readModbus[8] = lengths;   // Length
  // Data modbus
  for (i = 9; i <= 76; i++)
    modbusTcpResponse_readModbus[i] = data_TCP[i];
  // Data id RFID
  for (i = 77; i <= 80; i++)
    modbusTcpResponse_readModbus[i] = id_TCP[i - 77];
    
  Serial.println();
  if (dataModbusTcp[8] == 0x75) {
    for (j = 0; j <= 8; j++)
    {
      client.write(modbusTcpResponse_readModbus[j]);
      //Serial.print(modbusTcpResponse_readModbus[j], HEX);  Serial.print(" ");
    }
    i = ((dataModbusTcp[9] - 0x30) * 2) + 9;
    for (j = i ; j < (i + lengths); j++)
    {
      client.write(modbusTcpResponse_readModbus[j]);
      Serial.print(modbusTcpResponse_readModbus[j], HEX);  Serial.print(" ");
    }
...
void setFN06(byte dataModbusTcp[]) {
  ...
  Serial.println("FN 06");
  memset(_bufferFN05, 0, sizeof(_bufferFN05));
  memset(_bufferFN06, 0, sizeof(_bufferFN06));
  memset(modbusTcpResponse, 0 , sizeof(modbusTcpResponse));
  uint16_t address = dataModbusTcp[8] << 8 | dataModbusTcp[9];
  uint16_t en = dataModbusTcp[10] << 8 | dataModbusTcp[11];
  if (address == 0x754F)
  {
    address = 0x0FA0;
  }
  else if (address == 0x7550)
  {
    address = 0x55F0;
  }
  else if (address == 0x7551)
  {
    address = 0x4E20;
    ...
    vTaskDelay(10 / portTICK_PERIOD_MS);
    if (modbusEV.writeSingleCoil(address, en, _bufferFN05))
    {
      for (uint8_t n = 0; n <= 3; n++)
        modbusTcpResponse[n] = dataModbusTcp[n];

      modbusTcpResponse[4] = 0x00;   // Length field high byte
      modbusTcpResponse[5] = 0x06;   // Length field low byte
      modbusTcpResponse[6] = dataModbusTcp[6];   // Slave
      modbusTcpResponse[7] = dataModbusTcp[7];   // Function
      modbusTcpResponse[8] = dataModbusTcp[8];    // address High
      modbusTcpResponse[9] = dataModbusTcp[9];    // address Low

      if (_bufferFN05[4] == 0xFF && _bufferFN05[5] == 0x00)
      {
        modbusTcpResponse[10] = 0x00;     // data return byte high
        modbusTcpResponse[11] = 0x01;     // data return byte low
      }
      else if (_bufferFN05[4] == 0x00 && _bufferFN05[5] == 0x00)
      {
        modbusTcpResponse[10] = 0x00;     // data return byte high
        modbusTcpResponse[11] = 0x00;     // data return byte low
      }


      for (uint8_t j = 0 ; j <= 11; j++)
      {
        client.write(modbusTcpResponse[j]);
        Serial.print(modbusTcpResponse[j], HEX);
      }
    }
    return;
  }
  ...
  vTaskDelay(10 / portTICK_PERIOD_MS);
  if (modbusEV.setfn06(address, en, _bufferFN06))
  {
    for (uint8_t n = 0; n <= 3; n++)
      modbusTcpResponse[n] = dataModbusTcp[n];
    ...
    modbusTcpResponse[8] = dataModbusTcp[8];    // address High
    modbusTcpResponse[9] = dataModbusTcp[9];    // address Low
    modbusTcpResponse[10] = _bufferFN06[4];     // data return byte high
    modbusTcpResponse[11] = _bufferFN06[5];     // data return byte low
...
void setFN16(byte dataModbusTcp[]) {
  ...
  Serial.println("FN16");
  memset(_buffers1, 0, sizeof(_buffers1));
  memset(_buffers2, 0, sizeof(_buffers2));
  memset(_buffers3, 0, sizeof(_buffers3));
  memset(modbusTcpResponse, 0 , sizeof(modbusTcpResponse));
  uint16_t address = dataModbusTcp[8] << 8 | dataModbusTcp[9];
  uint16_t lengths = dataModbusTcp[10] << 8 | dataModbusTcp[11];
  uint16_t datas;
  ...
  switch (address)
  {
    case 0x754F:
      ...
      {
        case 0x03 :
          datas = dataModbusTcp[17] << 8 | dataModbusTcp[18];
          if (datas == 0x0001)
            datas = 0xFF00;
          else if (datas == 0x0001)
            datas = 0x0000;
          vTaskDelay(10 / portTICK_PERIOD_MS); // Address 20000
          if (modbusEV.writeSingleCoil(0x4E20, datas, _buffers1)) {
          }
        case 0x02 :
          datas = dataModbusTcp[15] << 8 | dataModbusTcp[16];
          vTaskDelay(10 / portTICK_PERIOD_MS);   // Address 22000
          if (modbusEV.setfn06(0x55F0, datas, _buffers2)) {
          }
        case 0x01 :
          datas = dataModbusTcp[13] << 8 | dataModbusTcp[14];
          vTaskDelay(10 / portTICK_PERIOD_MS);    // Address 4000
          if (modbusEV.setfn06(0x0FA0, datas, _buffers3)) {
          }
          ...
          modbusTcpResponse[8] = dataModbusTcp[8];    // address High
          modbusTcpResponse[9] = dataModbusTcp[9];    // address Low
          modbusTcpResponse[10] = dataModbusTcp[10];
          modbusTcpResponse[11] = dataModbusTcp[11];
        ...
        default:
          illegalDataAddress(dataModbusTcp);
          break;
      }
      break;
    case 0x7550:
...
void illegalDataAddress(byte dataModbusTcp[]) {
  ...
  modbusTcpResponse[4] = 0x00;   // Length field high byte
  modbusTcpResponse[5] = 0x03;   // Length field low byte
  modbusTcpResponse[6] = dataModbusTcp[6];   // Slave
  modbusTcpResponse[7] = 0x83;    //dataModbusTcp[7];   // Function
  modbusTcpResponse[8] = 0x02;   // Length
...
void illegalDataFunction(byte dataModbusTcp[]) {
  ...
  modbusTcpResponse[6] = dataModbusTcp[6];   // Slave
  modbusTcpResponse[7] = 0x83;    //dataModbusTcp[7];   // Function
  modbusTcpResponse[8] = 0x01;   // Length
...
void setRFID(byte dataModbusTcp[]) {
  Serial.println("Set RFID");
  uint16_t address = dataModbusTcp[8] << 8 | dataModbusTcp[9];
  uint16_t lengths = dataModbusTcp[10] << 8 | dataModbusTcp[11];
  uint16_t datas;

  if (address == 0x7552 && lengths == 2
      && dataModbusTcp[13] == 0xFF && dataModbusTcp[14] == 0xFF
      && dataModbusTcp[15] == 0xFF & dataModbusTcp[16] == 0xFF)
  {
    flag_RFID = true;
  ...
  else if (address == 0x7552 && lengths == 2
           && dataModbusTcp[13] == 0x00 && dataModbusTcp[14] == 0x00
           && dataModbusTcp[15] == 0x00 & dataModbusTcp[16] == 0x00)
  {
    flag_RFID = false;
    memset(id_TCP, 0, sizeof(id_TCP));
    for (uint8_t n = 0; n <= 3; n++)
      modbusTcpResponse[n] = dataModbusTcp[n];
...
void setIP(byte dataModbusTcp[]) {
  ...
  if (address == 0x7554 && lengths == 2 )
  {
    for (uint8_t i = 0; i < 4; i++)
    {
      EEPROM.write(i, dataModbusTcp[i + 13]);
    }
    EEPROM.commit();
...
void restartEV(byte dataModbusTcp[]) {
  Serial.print("FN 05 Restart EV");
  uint16_t values = dataModbusTcp[10] << 8 | dataModbusTcp[11];
  Serial.print("Value = "); Serial.println(values, HEX);
  Serial.println("Before wait");
  while (flag_loop)
  {
    Serial.println("Wait loop");
  ...
  if (modbusEV.writeSingleCoil(0x5208, values , _bufferRestart))
  {
    for (uint8_t n = 0; n <= 3; n++)
      modbusTcpResponse[n] = dataModbusTcp[n];
    ...
    modbusTcpResponse[8] = dataModbusTcp[8];    // address High
    modbusTcpResponse[9] = dataModbusTcp[9];    // address Low
    modbusTcpResponse[10] = _bufferRestart[4];
    modbusTcpResponse[11] = _bufferRestart[5];
...
void restartESP(byte dataModbusTcp[]) {
  Serial.print("FN 05 Restart ESP");
  uint16_t values = dataModbusTcp[10] << 8 | dataModbusTcp[11];
  Serial.print("Value = "); Serial.println(values, HEX);
  for (uint8_t n = 0; n <= 3; n++)
    modbusTcpResponse[n] = dataModbusTcp[n];
  ...
  if(dataModbusTcp[10] == 0xFF && dataModbusTcp[11] == 0x00)
  {
